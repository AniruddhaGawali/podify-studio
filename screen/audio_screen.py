...
class AudioScreen(customtkinter.CTkFrame):
    def __init__(self, master: customtkinter.CTk, font: font.FontSize,podcast:Podcast):
        ...
        self.options = [item for sublist in [TextToAudio.allVoiceInGTTS(), TextToAudio.allVoiceInPyttsx3()] for item in sublist]
    ...
    def print_script(self):
        print(self.podcast.get_person_from_script())
    ...
    def add_persons(self, persons: List[str]):
       ...
       for i in persons:
            ...
            form = [
                customtkinter.CTkLabel(self, text=i+":", font=self.font.md).grid(row=self.current_row, column=1, pady=15, padx=30, sticky="w"),
                customtkinter.CTkOptionMenu(self, variable=tk.StringVar(self).set(values[0]), command=self.optionmenu_callback, values=values).grid(row=self.current_row, column=2, pady=15, padx=30, columnspan=1),
            ]
